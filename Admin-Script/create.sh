...
envsubst < project.yaml | kubectl --context cci create -f -

# Created by an admin to assign a role per user. cci will validate one resource per user.
envsubst < projectrolebinding.yaml | kubectl --context cci create -f -

# Created by an admin to group one or many supervisors in a region.
envsubst < region.yaml | kubectl --context cci create -f -
...
echo "---------------------------------------------------------------"
echo "STEP 1 : Get list of supervisors :  kubectl -n cci-config get supervisors"
echo "STEP 2 : Edit the superisor to add Region and Labels :  kubectl -n cci-config edit supervisor <Supervisor Name>"
echo "---------------------------------------------------------------"
...
envsubst < supervisornamespaceclass.yaml | kubectl create --context cci -f -
...
envsubst < supervisornamespaceclassconfig.yaml | kubectl create --context cci -f -
